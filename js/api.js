...
// ── SharePoint (autodétection par compte Microsoft) ────────────────────────────
// ── Journal applicatif ────────────────────────────────────────────────────────
const JournalApi = {
...
const SharePointApi = {
  status:    ()                              => apiRequest('sharepoint_status'),
  // Disponibilité d'un lien donné + métadonnées de repli (jamais le contenu).
  check:     (docId)                         => apiRequest(`sharepoint_check&id=${encodeURIComponent(docId)}`),
  tenant:    ()                              => apiRequest('sharepoint_tenant'),
  sites:     (q = '', top = 0)               => apiRequest(`sharepoint_sites${q ? '&q='+encodeURIComponent(q) : ''}${top ? '&top='+encodeURIComponent(top) : ''}`),
  mydrives:  ()                              => apiRequest('sharepoint_mydrives'),
  drives:    (siteId)                        => apiRequest(`sharepoint_drives&siteId=${encodeURIComponent(siteId)}`),
  ...
  link:      (entiteType, entiteId, file)    => apiRequest('sharepoint_link', 'POST', {
    ...
    // stable (résolution insensible aux renommages). Fourni par la liste des
    // fichiers ; repli sur le contexte de navigation SharePoint courant.
    driveId: file.driveId || (typeof _spContext !== 'undefined' && _spContext ? (_spContext.driveId || '') : ''),
